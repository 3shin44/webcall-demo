<template>
  <div class="instruct-modal">

    <button type="button" data-bs-toggle="modal" data-bs-target="#instructModal" :class="{'min-width-50' : iconCode == 2 }" >
      <img class="w-100 d-block" :src="iconSrc[iconCode]" alt="info" >
    </button>

    <!-- Modal -->
    <div class="modal fade" id="instructModal" tabindex="-1" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">操作說明</h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
          </div>
          <div class="modal-body">
            <div class="steps">
              <h6 class="fw-bold">操作步驟</h6>
              <ol>
                <li><span>請裝置好麥克風及喇叭(或耳機)。</span> </li>
                <li><span>請點選綠色「通話」鍵，系統將自動為您接通，雙方開始通話。</span></li>
                <li><span>通話結束請點選紅色「結束」鍵，雙方結束通話。</span></li>
              </ol>
            </div>
            <div class="warning">
              <h6 class="fw-bold">注意事項</h6>
              <ol>
                <li><span>請使用瀏覽器 Chrome、Edge、Safari、Firefox以上最新版本，以避免無法正常操作。</span></li>
                <li><span>若您一直響鈴無法通話，可能因為網路安全限制（例如防火牆阻擋），建議您改用一般電話撥打0800-081-108按888，或手機撥打 (02) 2171-1055按888，與電銷專員通話。</span></li>
              </ol>
            </div>
          </div>
        </div>
      </div>
    </div>

  </div>
</template>

<script>
  export default {
    name: 'InstructModal',
    props: {
      iconCode:{
        Type: Number,
        default: 1
      }
    },
    data() {
      return {
        // 圖示來源
				iconSrc: {
					1: require("../../public/images/gd1.png"),
					2: require("../../public/images/gd2.svg")
				}
      }
    },
  }
</script>


<style scoped>
  .instruct-modal>button{
    padding: 0;
    max-width: 100px;
    background-color: transparent;
    border: none;
    transition: .5s;
    height: 35px;
  }

  .instruct-modal .modal-dialog{
    top: 10%;
  }

  /* 方形圖示 */
  .min-width-50{
    height: 35px;
    width: 35px;
  }
</style>